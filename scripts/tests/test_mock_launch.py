...
def launch_mock(wrist_motor_type="EL05", wait_sec=15):
    """启动 Mock 硬件并等待控制器就绪"""
    global LAUNCH_PROC
    kill_launch()
...
def check_controllers_active(expected_active):
    ...
    for name in expected_active:
        if name in out and "active" in out.split(name)[1].split("\n")[0]:
            results[name] = True
...
def check_robot_description(motor_type, effort_val):
    ...
    out, _, rc = run_cmd("ros2 topic echo /robot_description --once", timeout=10)
    if rc != 0 or not out:
        log_fail("robot_description 话题不可用")
        return False

    if f">{motor_type}<" in out:
...
def test_a33_controller_switch():
    ...
    out, _, _ = run_cmd("ros2 control list_controllers")
    if "zero_torque_controller" in out and "active" in out.split("zero_torque_controller")[1].split("\n")[0]:
