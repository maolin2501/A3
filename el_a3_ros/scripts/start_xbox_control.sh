#!/bin/bash
# NOTE: el_a3_teleop was removed from this repo; the teleop launch at the end is disabled.
...
SDK_DETECT_SCRIPT="$PROJECT_ROOT/../el_a3_sdk/scripts/detect_controller.py"

# Enter workspace (ROS packages at el_a3_ros root)
cd "$PROJECT_ROOT"
...
if [ -e /dev/input/js0 ]; then
    ...
    if [ -n "$XBOX_EVENT" ]; then
        ...
        JS_DEVICE=""
        CONTROLLER_PROFILE="generic_hid"
        CONTROLLER_NAME="Unknown event device"
...
if [ -n "$JS_DEVICE" ] && [ -f "$SDK_DETECT_SCRIPT" ]; then
    eval "$(python3 "$SDK_DETECT_SCRIPT" --device "$JS_DEVICE" --format shell)"
    echo -e "${GREEN}✓${NC} Detected controller: ${CONTROLLER_NAME:-unknown}"
    echo -e "${GREEN}✓${NC} Selected profile: ${CONTROLLER_PROFILE:-generic_hid}"
    echo ""
fi
...
echo "  Controller profile: ${CONTROLLER_PROFILE:-generic_hid}"
echo ""
echo "Press Ctrl+C to stop the system"
echo ""
sleep 2

echo "Error: el_a3_teleop is not in this workspace (package removed)."
echo "Future teleop launch should receive: controller_profile:=${CONTROLLER_PROFILE:-generic_hid}"
exit 1
