#!/bin/bash
# NOTE: el_a3_teleop was removed from this repo; the final launch step is disabled below.
...
if [ -f "$SDK_DETECT_SCRIPT" ]; then
    eval "$(python3 "$SDK_DETECT_SCRIPT" --device "$JS_DEVICE" --format shell)"
    echo -e "${GREEN}✓ Detected controller: ${CONTROLLER_NAME:-unknown}${NC}"
    echo -e "${GREEN}✓ Selected profile: ${CONTROLLER_PROFILE:-generic_hid}${NC}"
    echo ""
fi
...
echo "Error: el_a3_teleop is not in this workspace (package removed). Wire this script to your teleop launch when available."
