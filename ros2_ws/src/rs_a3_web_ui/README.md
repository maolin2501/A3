...
基于 Flask + Three.js 的 RS-A3 机械臂 Web 控制界面。
...
# 运行服务器
python3 src/rs_a3_web_ui/rs_a3_web_ui/web_server.py --port 5000
```
...
# 先构建包
cd /home/wy/RS/A3
colcon build --packages-select rs_a3_web_ui

# 启动
source install/setup.bash
ros2 launch rs_a3_web_ui web_ui.launch.py
```
...
| 服务 | 类型 | 说明 |
|------|------|------|
| `/rs_a3/set_zero_torque_mode` | `std_srvs/SetBool` | 零力矩模式/电机使能 |
...
1. **启动顺序**：先启动机械臂控制系统 (`rs_a3_control.launch.py`)，再启动Web UI
...
1. 检查`/rs_a3/set_zero_torque_mode`服务是否可用
2. 重启机械臂控制系统
